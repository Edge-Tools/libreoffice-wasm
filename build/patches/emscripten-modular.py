...
# Every substitution below is guarded: if upstream drifts and a pattern no
# longer matches, the build must fail loudly here rather than silently produce
# an unpatched (non-modular, auto-running) module.

# 1. Add MODULARIZE / EXPORT_ES6 / INVOKE_RUN after the existing EXIT_RUNTIME=0
...
# 2. Replace the Qt6-conditional callMain/FS exports with an unconditional set.
#    Handles both old form ("HEAPU32"$(if $(ENABLE_QT6),...]) and new form
#    ("throwBindingError"$(if $(ENABLE_QT6),...]).
c, n = re.subn(
    r'\$\(if \$\(ENABLE_QT6\),.*?\]',
    ',"callMain","FS","ENV"]',
    c,
